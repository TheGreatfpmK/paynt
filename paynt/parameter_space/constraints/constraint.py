"""Constraint: the shared extension point for custom SMT constraints over the parameter space, usable by
both SynthesizerSMPMC (natively -- a constraint can reference the viable(...) predicate, e.g. to wrap it
in a quantifier for robust synthesis) and CEGIS's SmtSolver (structural constraints only, no viable -- see
...
class ConstraintContext:
    """Everything a Constraint needs to emit Z3 clauses. One shape for both engines: CEGIS's SmtSolver and
    SMPMC's ParameterBitVecEncoding both build one z3.BitVec per parameter through the same
    ...
    def base_clauses(self) -> list[Any]:
        """in_range(), plus viable(...) when available -- every concrete Constraint is self-contained
        (mirroring molehill's own constraints, which each independently include their equivalent of these
        two rather than composing with a separate "exists" constraint), so every Constraint.build() should
        start its returned list from this rather than reimplementing it."""
    ...
    def requires_viable(self) -> Any:
        """Call from a constraint that needs to reference the viable(...) predicate (e.g. to wrap it in a
        quantifier for robust synthesis). Raises a clear, actionable error rather than a confusing
        AttributeError/TypeError when used from an engine (CEGIS) that has no such predicate."""
...
class Constraint:
    """Base class for a custom constraint over the parameter space, contributing additional Z3 clauses
    beyond the plain per-parameter domain (ConstraintContext.in_range()). Register a new constraint in
    paynt/parameter_space/constraints/_registry.py's CONSTRAINTS mapping."""

    name: ClassVar[str]

    def build(self, ctx: ConstraintContext) -> list[Any]:
        """Return the additional Z3 clauses this constraint contributes. Always a list (never a bare
        expression, unlike molehill's inconsistent build_constraint), consumed via solver.add(*clauses)
        (SMPMC) or z3.And(encoding, *clauses) (CEGIS)."""
