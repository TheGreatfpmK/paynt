...
def observations(info: PomdpInfo) -> int:
    return info.pomdp.nr_observations


def collect_policy(info: PomdpInfo, dtmc: Any, mc_result: Any, specification: paynt.specification.property.Specification) -> list[list[dict[int, Any]]]:
    # TODO: move to a util file once SAYNT is reworked -- this interprets a model-checking result back
    # into a POMDP policy, it doesn't need to live here
    ...
    policy = []
    for obs in range(observations(info)):
        mem_size = info.observation_memory_size[obs]
    ...
    for dtmc_state in range(dtmc.states):
        ...
        pomdp_state = info.pomdp_manager.state_prototype[mdp_state]
        memory_node = info.pomdp_manager.state_memory[mdp_state]
        observation = info.pomdp.get_observation(pomdp_state)
...
def extract_policy(
    ...
    return collect_policy(cast(PomdpInfo, colored_mdp.feature_info), dtmc, mc_result, specification)
...
def policy_size(colored_mdp: paynt.colored_mdp.ColoredMdp, assignment: paynt.parameter_space.parameter_space.ParameterSpace) -> int:
    ...
    size_gamma = sum(info.observation_memory_size)  # explicit

    if not info.posterior_aware:
        # size of update function delta of a posterior-unaware FSC:
        #   for each memory node, a list of prior-update pairs
        size_delta = sum(info.observation_memory_size)  # explicit
    ...
    max_mem = max(info.observation_memory_size)
    memory_prior_posteriors: list[list[set[int]]] = [[set() for _ in range(observations(info))] for _ in range(max_mem)]
    for state in range(dtmc.states):
        ...
        pomdp_state = info.pomdp_manager.state_prototype[mdp_state]
        memory_node = info.pomdp_manager.state_memory[mdp_state]
        prior = info.pomdp.get_observation(pomdp_state)
        ...
        for entry in dtmc.model.transition_matrix.get_row(state):
            ...
            pomdp_successor = info.pomdp_manager.state_prototype[mdp_successor]
            posterior = info.pomdp.get_observation(pomdp_successor)
...
def get_parameter_space_pomdp(info: PomdpInfo, mdp: Any) -> Any:
    ...
    full_observ_list = []
    for state in range(info.pomdp.nr_states):
        obs = info.pomdp.get_observation(state)
        for mem in range(info.observation_memory_size[obs]):
...
def assignment_to_fsc(info: PomdpInfo, assignment: paynt.parameter_space.parameter_space.ParameterSpace) -> FscFactored:
    assert assignment.size == 1, "expected parameter space of size 1"
    num_nodes = max(info.observation_memory_size)
    num_observations = observations(info)
    fsc = FscFactored(num_nodes, num_observations, is_deterministic=True)
    fsc.observation_labels = info.observation_labels

    # collect action labels
    action_labels_set = set()
    for labels in info.action_labels_at_observation:
    ...
    observation_to_actions: list[list[int]] = [[] for obs in range(num_observations)]
    for obs, action_labels in enumerate(info.action_labels_at_observation):
    ...
    for obs, parameters in enumerate(info.observation_action_parameters):
        for node, parameter in enumerate(parameters):
            option = assignment.parameter_options(parameter)[0]
            action_label = info.action_labels_at_observation[obs][option]
            action = action_label_indices[action_label]
            fsc.action_function[node][obs] = action
    for obs, parameters in enumerate(info.observation_memory_parameters):
        for node, parameter in enumerate(parameters):
            option = assignment.parameter_options(parameter)[0]
...
def get_induced_dtmc_from_fsc(info: PomdpInfo, fsc: FscFactored) -> Any:
    ...
    state_queue = [(info.pomdp.initial_states[0], 0)]
    dtmc_states_map[len(dtmc_states_map)] = (info.pomdp.initial_states[0], 0)

    while state_queue:
        current_state_memory_pair = state_queue.pop()

        # compute the successor states
        current_obs = info.pomdp.observations[current_state_memory_pair[0]]
        ...
        for selected_action in selected_actions.keys():
            for selected_update in selected_updates.keys():
                ...
                choice_offset_for_selected_label = info.action_labels_at_observation[current_obs].index(selected_action_label)
                choice_index = info.pomdp.get_choice_index(current_state_memory_pair[0], choice_offset_for_selected_label)
    ...
    state_action_rewards: dict[str, list[Any]] = {name: [] for name in info.pomdp.reward_models.keys()}
    for dtmc_state, current_state_memory_pair in dtmc_states_map.items():
        current_obs = info.pomdp.observations[current_state_memory_pair[0]]
        selected_actions = action_function[current_state_memory_pair[1]][current_obs]
        if selected_actions is None:
            for reward_name in info.pomdp.reward_models.keys():
        ...
        current_reward = dict.fromkeys(info.pomdp.reward_models.keys(), 0)
        ...
        for selected_action, action_prob in selected_actions.items():
            ...
            for reward_name, reward_model in info.pomdp.reward_models.items():
    ...
    for label in info.pomdp.labeling.get_labels():
        dtmc_labeling.add_label(label)
    for dtmc_state, current_state_memory_pair in dtmc_states_map.items():
        for label in info.pomdp.labeling.get_labels_of_state(current_state_memory_pair[0]):
            if label == "init" and current_state_memory_pair != (info.pomdp.initial_states[0], 0):  # only (0,0) state is initital
                continue
    ...
    for reward_name in info.pomdp.reward_models.keys():
        ...
        assert info.pomdp.reward_models[reward_name].has_state_action_rewards, "currently this implementation expects state action rewards"
...
def compute_qvalues(
    ...
    info = cast(PomdpInfo, colored_mdp.feature_info)
    # model check
    submdp = colored_mdp.build_assignment(assignment)
    ...
    state_memory_value_total = [[None for memory in range(memory_size)] for state in range(info.pomdp.nr_states)]
    for state in range(info.pomdp.nr_states):
        for memory in range(memory_size):
            value = state_memory_value[(state, memory)]
            if value is None:
                obs = info.pomdp.observations[state]
                if memory < info.observation_memory_size[obs]:
...
def next_belief(info: PomdpInfo, belief: dict[int, float], action_label: str, next_obs: int) -> dict[int, float]:
    any_belief_state = list(belief.keys())[0]
    obs = info.pomdp.observations[any_belief_state]
    action = info.action_labels_at_observation[obs].index(action_label)
    new_belief_acc: dict[int, float] = collections.defaultdict(float)
    info.pomdp.nondeterministic_choice_indices.copy()
    for state, state_prob in belief.items():
        choice = info.pomdp.get_choice_index(state, action)
        for entry in info.pomdp.transition_matrix.get_row(choice):
            next_state = entry.column
            if info.pomdp.observations[next_state] == next_obs:
