...
def family_colored_mdp_factory():
    """A sketch whose PRISM program is an MDP (not a DTMC) with parameters resolves to a ColoredMdp with feature_kind "family" --
...
def family_task():
    ...
    family-specific MdpFamilyTask, not the base task fields like disable_expected_visits)."""
    sketch_path, props_path = get_sketch_paths("tests/mdp-family-avoid-8-2-easy")
...
def pomdp_family_colored_mdp_factory():
    """A sketch whose PRISM program is a POMDP with parameters resolves to a ColoredMdp with feature_kind "pomdp_family" -- here the
