...
class TestDecPomdpColoredMdpFactory:
    ...
    def test_set_agent_imperfect_memory_size_produces_a_fresh_colored_mdp(self, decpomdp_colored_mdp_factory):
        """Per-agent memory sizing: distinct from set_imperfect_memory_size, which resizes every agent. This
        is a partial update over an existing per-agent baseline, not a standalone first build -- build()
        establishes that baseline for every agent first, matching how any real caller would need to use it."""
