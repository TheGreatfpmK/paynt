...
@pytest.fixture
def pomdp_task():
    """The SynthesisTask sibling of pomdp_colored_mdp_factory -- see tests/dt/conftest.py's dt_task for why this loads the sketch itself rather than reaching
    into the factory."""
    sketch_path, props_path = get_sketch_paths("tests/pomdp-maze")
...
def posmg_colored_mdp_factory():
    """A POSMG sketch resolves to a PosmgColoredMdp via PosmgColoredMdpFactory.

    mec-test has only single-state (perfect-information) observations for the optimizing player, so it is fast but never
...
def posmg_test_game_colored_mdp_factory():
    """Unlike mec-test, test-game's optimizing player has multi-state observations, so this is the fixture that actually exercises memory unfolding (see
    test_synthesize_improves_with_memory_unfolding)."""
